...
Then("no page's HTML should exceed 30 KB, its scripts 10 KB, or its styles 25 KB, except that the Admin pages' scripts may reach 30 KB", function () {
  const problems = [];
  for (const { route, page } of this.data.pages) {
    ...
    if (kb(js) > (/\/admin\/$/.test(route) ? 30 : 10)) problems.push(`${route}: scripts ${kb(js).toFixed(1)} KB`);
