...
Then('the photo tooling should be the only code that contacts R2', function () {
  ...
  const fetchers = listSourceFiles()
    .filter(({ text, path }) => /\bfetch\s*\(/.test(text) && !/GeoRedirect|geo\.ts|contact\.astro|results-viewer\.ts|admin-common\.ts/.test(path))
    .map((f) => f.path);
  assert.deepEqual(fetchers, [], 'Unexpected network calls in site source (only location detection, the contact form and the Admin viewers (through admin-common.ts) may fetch, at runtime in the browser)');
});
